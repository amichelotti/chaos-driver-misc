 ...
 */
using namespace driver::misc;
static int checkData(ChaosUniquePtr<ChaosDatasetIO>& test, std::vector<ChaosDataSet> &res, uint64_t &pcktmissing, uint64_t &pckt, uint64_t &pcktreplicated, uint64_t &pcktmalformed, uint64_t &badid)
{
    int reterr = 0;
...
for (uint32_t point_cnt = npoints,incr=2; point_cnt <= pointmax; (incr==0)?(point_cnt+=pointincr):(point_cnt=(pow(pointincr,incr))),incr++)
    {
		...
		/*                if ((cnt % tenpercent) == 0)
            ...
            if (pagelen == 0)
            {
                ...
                std::cout << "["<<name<<"] retrived:"<<res.size()<<" items, items/s:"<<pull_avg<<" time:"<<pull_time<<std::endl;
                total = res.size();
                err += checkData(test, res, pckmissing, pckt, pcktreplicated, pckmalformed, badid);
            }
            else
            {
                ...
                while (test->queryHasNext(uid))
                {
                    ...
                    std::cout << "["<<name<<"] retrived:"<<res.size()<<"/"<<total<<" items , items/s:"<<pull_avg<<" time:"<<pull_time<<std::endl;
            ...
            if (total != loops)
            {
                std::cout <<"["<<name<<"] # number of data retrived " << total << " different from expected:" << loops << std::endl;
                err++;
            }
            if (err != 0)
            {
                std::cout <<"["<<name<<"] ## Total errors:" << err << " missing packets:" << pckmissing << " replicated:" << pcktreplicated << " pcktmalformed:" << pckmalformed << " badrunid:" << badid << std::endl;
            }
            else
            { 
	...
	test->deinit();
        if(++thread_done==nthreads){
            ...
            cond.notify_all();
                ...
                //std::cout <<"["<<name<<"] waiting:" << thread_done<<" points:"<<point_cnt<<std::endl;
                cond.wait(mutex_thread);
                cond.notify_all();
                //std::cout <<"["<<name<<"] restart:" << thread_done<<" points:"<<point_cnt<<std::endl;
...
int main(int argc, const char **argv)
{
    ...
    ChaosMetadataServiceClient::getInstance()->getGlobalConfigurationInstance()->addOption("nthreads", po::value<uint32_t>(&nthreads)->default_value(nthreads), "Number of concurrent accesses");
    ...
    for(int cnt=0;cnt<nthreads;cnt++){
        ...
        ss<<name<<"_"<<cnt;
        params[cnt].thid=cnt;
        workers[cnt]=new boost::thread(boost::bind(&performTest,ss.str(),boost::ref(params[cnt])));
    }
    sleep(5);
    for(int cnt=0;cnt<nthreads;cnt++){
        workers[cnt]->join();
