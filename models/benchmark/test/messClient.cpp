...
public:
  ...
  int test(int bytes,uint64_t delay,int repetition) {
	...
	if(prof && (size==sizeof(MessProfilePacketInfo))){
	  LDBG_<<"cu count:"<<prof->uidx<<" crc:"<<prof->crc32<<" local counter:"<<counter<<" size "<<size;
	  if((prof->uidx == (repetition-1)) && (counter ==0)){
	    continue;
	  }
	  ...
	  *fs << bytes << "," << total_micro/ui_cycles << "," << micro_min << "," << micro_max<< ","<<cycle_us<<","<<prof->tprof.cycle_us<<","<<prof->tprof.cycle_sigma<<","<<kbs<<","<<ui_packets_lost<<","<<(float)cu_ui_timeshift*1.0/((ok_counter==0)?1:ok_counter)<<","<< (int64_t)prof->tprof.cmd_arrival_time_us - (int64_t)start_test.time_of_day().total_microseconds()<<","<<prof->tprof.cycle_sec<<endl;
	  //sleep(1);
	  return 0;
