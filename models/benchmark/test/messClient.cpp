...
public:
  perform_test(string _test_name, string _fradix_name, chaos::metadata_service_client::node_controller::CUController *ctrl) : test_name(_test_name), fradix_name(_fradix_name), controller(ctrl)
  {
    ...
    if (readonly && !test_ui_freq)
      return;
    std::string dname;
    controller->getDeviceId(dname);
    LDBG_ << "initializing " << dname;
    err = controller->initDevice();
    if (err == ErrorCode::EC_TIMEOUT)
    {
      LERR_ << "Exception initializing";
      throw CException(6, string(__FUNCTION__), "Set device to init state");
    }
    sleep(1);
    LDBG_ << "starting " << dname;

    err = controller->startDevice();
    if (err == ErrorCode::EC_TIMEOUT)
    {
      LERR_ << "Exception starting";

      throw CException(2, string(__FUNCTION__), "Set device to start state");
    }
...
public:
  ...
  int test(int bytes, uint64_t delay, int repetition)
  {
    ...
    LAPP_ << "Performing  " << test_name << "bytes :" << bytes << " scheduling :" << delay << " us"
    ...
    retry = 10;
   LDBG_ << "readonly:"<<readonly<<",wronly:"<<wronly<<",test_ui_freq:"<<test_ui_freq;

    if ((readonly == false) || (wronly == true) || test_ui_freq){
      do
      {
        start_test_us = start_test.time_of_day().total_microseconds();
        LDBG_ << "submitting command:calc_bandwidth";
        ...
        {
          usleep(1000);
          LERR_ << "sending command resending for " << retry;
        }
      } while ((err != ErrorCode::EC_NO_ERROR) && retry--);

      if (err != ErrorCode::EC_NO_ERROR)
        throw CException(2, "Error", "executing command");
    }
    ...
    do
    {
      ...
      if (wrapped_data)
      {
        ...
        if ((readonly == false) && prof && (size == sizeof(MessProfilePacketInfo)) && (prof->ts_tag == start_test_us))
        {
...
int main(int argc, const char *argv[])
{
  try
  {
    ...
    if (!ChaosMetadataServiceClient::getInstance()->getGlobalConfigurationInstance()->hasOption(OPT_MESS_DID))
      throw CException(1, "invalid device identification string", "check param");
    if (mess_device_id.size() == 0)
      throw CException(1, "invalid device identification string", "check param");
    ...
    controller->setRequestTimeWaith(5000);
    controller->setupTracking();
    if (!controller)
      throw CException(4, "Error allocating device controller", "device controller creation");
