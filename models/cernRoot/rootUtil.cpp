...
*/
/***************/
chaosBranch::chaosBranch(TTree *par, const std::string &key, const chaos::common::data::CDataWrapper &cd, const std::string &brsuffix) {
  cdkey             = key;
   ...
   */
    ...
    throw chaos::CException(-2, "zero size for key:" + name, __PRETTY_FUNCTION__);
  }
}
...
bool chaosBranch::add(const chaos::common::data::CDataWrapper &cd) {
  ...
  if (cd.isVector(cdkey)) {
    ...
    for (int cnt = 0; cnt < da->size(); cnt++) {
      ...
      memcpy((void *)addr,
             (void *)da->getRawValueAtIndex(cnt, siz),
             data_element_size);
    }
  } else {
...
ChaosToTree::ChaosToTree(TTree *rot, const std::string &_brsuffix)
    : root(rot), brsuffix(_brsuffix) {}
...
static TTree *
query_int(TTree *tree_ret, const std::string &chaosNode, const std::string &start, const std::string &end, const int channel, const std::string treeid, const std::string desc, int pageLen, bool multi, uint64_t runid, uint64_t pckid, const std::vector<std::string> tags = std::vector<std::string>()) {
  std::string brname;
  try {
    treeQuery_t  q;
...
static TTree *
query_int(TTree *tree_ret, const std::string &chaosNode, const std::string &start, const std::string &end, const int channel, const std::string treeid, const std::string desc, int pageLen, bool multi, const std::vector<std::string> tags = std::vector<std::string>()) {
...
TTree *queryChaosTree(TTree *tree_ret, const std::string &chaosNode, const std::string &start, const std::string &end, const int channel, const std::string &treeid, const std::string &desc, int pageLen) {
  return query_int(tree_ret, chaosNode, start, end, channel, treeid, desc, pageLen, true);
}
TTree *queryChaosTreeSB(const std::string &chaosNode, const std::string &start, const std::string &end, const int channel, const std::string &treeid, const std::string &desc, int pageLen) {
  ...
  if (ctrl) {
    ...
    if ((ret == 0)&& res.size()) {
      ...
      try {
        ChaosToTree      *branch = NULL;
        std::stringstream ss;
        ss << chaosNode << " TDB data";
        tree_ret = buildTree(chaosNode, ss.str());
        brname   = chaosNode;
        std::replace(brname.begin(),brname.end(),'/','_');
        branch   = new ChaosToTree(tree_ret, brname);
        if (branch == NULL) {
          ROOTERR << "Cannot create branches";
        ...
        int cnt=0;
        for (chaos::common::data::VectorCDWShrdPtr::iterator i = res.begin(); i != res.end(); i++,cnt++) {
          // addTree(q, i->get());
          if (i->get()) {
            ROOTDBG<<cnt<<"]"<<i->get()->getCompliantJSONString();
            branch->addData(*(i->get()));
          }
        }
        return tree_ret;
...
int32_t queryDBCount(const std::string &chaosNode, const std::string &start, const std::string &end, const ChaosStringSet &vars, const ChaosStringSet &tags) {
  ChaosController *ctrl = NULL;
  ctrl                  = new ChaosController();
  if (ctrl) {
    int32_t ret = ctrl->queryTSCount(chaosNode, start, end, tags, vars);
    delete ctrl;
    return ret;
  }
  return -1;
...
bool queryNextChaosTree(TTree *tree) {
  ...
  if (page != queries.end()) {
    ...
    if (ctrl) {
      ...
      queries[tree].pckid++;
      ROOTDBG << "START QUERY start:" << queries[tree].start << " end:" << queries[tree].end << " pckid:" << queries[tree].pckid << " runid:" << queries[tree].runid;
      int32_t ret = ctrl->queryHistory(queries[tree].start, queries[tree].end, queries[tree].runid, queries[tree].pckid, queries[tree].tags, queries[tree].channel, res, ChaosStringSet(), queries[tree].page);
      if (ret == 0) {
        last_size = res.size();
        totcnt += last_size;
        ...
        ROOTDBG << "QUERY RETURNED elements:" << last_size << " total:" << totcnt << " start:" << queries[tree].start << " end:" << queries[tree].end << " pckid:" << queries[tree].pckid << " runid:" << queries[tree].runid;

        again = (last_size == queries[tree].page);
      } else {
        ROOTERR << "## Error Query returned:" << ret;
      }
